...
% to co jest nizej zostało pobrane z instrukcji jak cos
%*****************************************************************************************
...
TF = 50*Tp; % wybór wartości stałej czasowej dla filtrów SVF
...
%*****************************************************************************************
% z instrukcji to co znajduje sie wyzej

Phi = [-yF(1:end-1) uF(1:end-1)];
PNLS_filtr = (Phi'*Phi)^-1*Phi'*ypF(2:end);
T_filtr = 1/PNLS_filtr(1);
kp_filtr = T_filtr*PNLS_filtr(2);
%kp_filtr = PNLS_filtr(2)/(1-exp(-Tp/T_filtr));
% Na podstawie parametrow powyzszych to sie liczy nizej
s = tf('s');
G_filtr = kp_filtr/(1+s*T_filtr); 
% albo to nizej albo to wyzej
%z = tf('z', Tp);
...
% Im wiekszy zakres to lepsza identyfikacja
